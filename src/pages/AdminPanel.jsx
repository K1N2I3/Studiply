              ...
              {passLoading ? (
                ...
                <>
                  {(() => {
                    ...
                    // Pagination
                    const totalPages = Math.ceil(filteredUsers.length / passItemsPerPage)
                        ...
                        {totalPages > 1 && (
                          <div className="flex items-center justify-between mt-6 pt-6 border-t border-gray-200">
                            <div className="flex items-center gap-2">
                              ...
                              <select
                                value={passItemsPerPage}
                                onChange={(e) => {
                                  setPassItemsPerPage(Number(e.target.value))
                                  setPassCurrentPage(1)
                                }}
                                className="ml-4 px-3 py-1.5 border border-gray-300 rounded-lg text-sm focus:ring-2 focus:ring-purple-500 focus:border-purple-500"
                              >
                            ...
                            <div className="flex items-center gap-2">
                              <button
                                onClick={() => setPassCurrentPage(prev => Math.max(1, prev - 1))}
                                disabled={passCurrentPage === 1}
                                className={`p-2 rounded-lg transition-all duration-200 ${
                                  passCurrentPage === 1
                                    ? 'bg-gray-100 text-gray-400 cursor-not-allowed'
                                    : 'bg-white border border-gray-300 text-gray-700 hover:bg-gray-50 hover:border-purple-500'
                                }`}
                              >
                              ...
                              <div className="flex items-center gap-1">
                                {Array.from({ length: Math.min(5, totalPages) }, (_, i) => {
                                  ...
                                  if (totalPages <= 5) {
                                    pageNum = i + 1
                                  } else if (passCurrentPage <= 3) {
                                    pageNum = i + 1
                                  } else if (passCurrentPage >= totalPages - 2) {
                                    pageNum = totalPages - 4 + i
                                  } else {
                                    pageNum = passCurrentPage - 2 + i
                                  }
                                  
                                  return (
                                    <button
                                      key={pageNum}
                                      onClick={() => setPassCurrentPage(pageNum)}
                                      className={`px-3 py-1.5 rounded-lg text-sm font-medium transition-all duration-200 ${
                                        passCurrentPage === pageNum
                              ...
                              <button
                                onClick={() => setPassCurrentPage(prev => Math.min(totalPages, prev + 1))}
                                disabled={passCurrentPage === totalPages}
                                className={`p-2 rounded-lg transition-all duration-200 ${
                                  passCurrentPage === totalPages
                                    ? 'bg-gray-100 text-gray-400 cursor-not-allowed'
                                    : 'bg-white border border-gray-300 text-gray-700 hover:bg-gray-50 hover:border-purple-500'
                                }`}
                              >
